...
impl<C: Currency, A: Amount> BestPathCalculator<C, A> for FloydWarshallCalculator {
    ...
    fn calc_best_paths(
        ...
        let currencies = pairs_and_prices
            .iter()
            .flat_map(
                |(
                    ProviderPair {
                        pair: Pair { source, target },
                        ..
                    },
                    ..,
                )| vec![source, target].into_iter(),
            )
            .collect::<BTreeSet<_>>()
            .into_iter()
            .collect::<Vec<_>>();
        let providers = pairs_and_prices
            .iter()
            .map(|(ProviderPair { provider, .. }, ..)| provider)
            .collect::<BTreeSet<_>>()
            .into_iter()
            .collect::<Vec<_>>();
        let currencies_by_ind = currencies
            .iter()
            .enumerate()
            .map(|(i, &x)| (x, i))
            .collect::<BTreeMap<_, _>>();
        let providers_by_ind = providers
            .iter()
            .enumerate()
            .map(|(i, &x)| (x, i))
            .collect::<BTreeMap<_, _>>();
            ...
            for (pp, cost) in pairs_and_prices {
                if c == &pp.pair.source {
                    graph.push(algo::Edge {
                        pair: algo::Pair {
                            source: currencies_by_ind[&pp.pair.source],
                            target: currencies_by_ind[&pp.pair.target],
                        },
        ...
        let res_map = res
            .into_iter()
            .map(
                |(algo::Pair { source, target }, algo::Path { total_cost, edges })| {
                    let pair = Pair {
                        source: currencies[source].clone(),
                        target: currencies[target].clone(),
                    };
                    let total_cost_u128 = (total_cost * PRECISION) as u128;
                    let path = PricePath {
                        total_cost: total_cost_u128.try_into().ok().unwrap(),
                        steps: edges
                            .into_iter()
                            .map(
                                |algo::Edge {
                                     pair: algo::Pair { source, target, .. },
                                     provider,
                                     cost,
                                 }| PathStep {
                                    pair: Pair {
                                    ...
                                    provider: providers[provider].clone(),
                                    cost: ((cost * PRECISION) as u128).try_into().ok().unwrap(),
                                },
                            )
                            .collect(),
                    };
...
mod tests {
    ...
    fn test_real_life_graph() {
        ...
        */
        ...
        .into_iter()
        .map(|(source, target, provider, cost)| {
            (
                ProviderPair {
                    pair: Pair {
                        source: source.as_str().as_bytes().to_vec(),
                        target: target.as_str().as_bytes().to_vec(),
                    },
                    provider,
                },
                (cost * PRECISION) as u128,
            )
        })
        .collect::<Vec<_>>();
        ...
        assert_eq!(
            vec![
                ...
                (
                    ...
                    "ETH".to_owned(),
                    0.1527,
                    vec![("BNB".to_owned(), "ETH".to_owned(), MOCK_PROVIDER, 0.1527)]
                ),
                (
                    "BNB".to_owned(),
                    "USDT".to_owned(),
                    364.19,
                    vec![("BNB".to_owned(), "USDT".to_owned(), MOCK_PROVIDER, 364.19)]
                ),
                (
                    "BTC".to_owned(),
                    "BNB".to_owned(),
                ...
                (
                    ...
                    "ETH".to_owned(),
                    15.09,
                    vec![("BTC".to_owned(), "ETH".to_owned(), MOCK_PROVIDER, 15.09)]
                ),
                (
                    "BTC".to_owned(),
                    "USDT".to_owned(),
                ...
                (
                    ...
                    "USDT".to_owned(),
                    17.43,
                    vec![("DOT".to_owned(), "USDT".to_owned(), MOCK_PROVIDER, 17.43)]
                ),
                (
                    "ETH".to_owned(),
                    "BNB".to_owned(),
                    6.548,
                    vec![("ETH".to_owned(), "BNB".to_owned(), MOCK_PROVIDER, 6.548)]
                ),
                (
                    "ETH".to_owned(),
                    "BTC".to_owned(),
                    0.06626,
                    vec![("ETH".to_owned(), "BTC".to_owned(), MOCK_PROVIDER, 0.06626)]
                ),
                (
                    "ETH".to_owned(),
                    "DOT".to_owned(),
                ...
                (
                    ...
                    "USDT".to_owned(),
                    2384.99,
                    vec![("ETH".to_owned(), "USDT".to_owned(), MOCK_PROVIDER, 2384.99)]
                ),
                (
                    "USDT".to_owned(),
                    "BNB".to_owned(),
                    0.002745,
                    vec![("USDT".to_owned(), "BNB".to_owned(), MOCK_PROVIDER, 0.002745)]
                ),
                (
                    "USDT".to_owned(),
                    "BTC".to_owned(),
                ...
                (
                    ...
                    "DOT".to_owned(),
                    0.05737,
                    vec![("USDT".to_owned(), "DOT".to_owned(), MOCK_PROVIDER, 0.05737)]
                ),
                (
                    "USDT".to_owned(),
                    "ETH".to_owned(),
